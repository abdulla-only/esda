...
description: >-
  ...
  create deck → add card → study queue → FSRS grade → unauthorized check. Use to confirm the
...
3. creates a deck (`POST /api/v1/decks`) and a card (`POST /api/v1/cards`),
4. fetches `GET /api/v1/study/queue`, grabs the card, and `POST /api/v1/study/grade`s
   it (rating 3 = Good), printing the updated FSRS state,
5. deletes the deck (cleanup) and asserts an unauthenticated
   `GET /api/v1/study/queue` returns **401**.
